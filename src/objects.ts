...
export function isValid(question: Question, answer: string): boolean {
    ...
    return question.options.reduce(
        (valid: boolean, option: string) =>
            option === answer || valid ? (valid = true) : (valid = false),
        false
    );
}
...
export function toMarkdown(question: Question): string {
    let fullQuestion: string = "# " + question.name + "\n" + question.body;
    if (question.type === "multiple_choice_question") {
        fullQuestion += "\n";
        fullQuestion += question.options.reduce(
            (allOptions: string, option: string) =>
